...
export default function LaporanBarangKembali({ barangKembaliData, lokasiList, filters }: LaporanBarangKembaliProps) {
    ...
    const applyFilters = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('laporan.kembali'), pickBy(data), { preserveState: true, replace: true });
    };

    const resetFilters = () => {
        reset();
        router.get(route('laporan.kembali'));
    };
            ...
            <div className="report-page px-4 py-6 md:px-6 lg:px-8">
                <div className="w-full">
                    ...
                    <div className="report-table">
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Info Pengembalian</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Info Barang</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status Kembali</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {barangKembaliData.data.length > 0 ? (
                                        barangKembaliData.data.map((item: BarangKembaliItem, index) => (
                                            <tr key={item.transaksi_id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                                    {(barangKembaliData.current_page - 1) * barangKembaliData.per_page + (index + 1)}
                                                </td>
                                                <td className="px-6 py-4 text-sm whitespace-nowrap">
                                                    <div className="font-semibold text-gray-900">{item.tanggal}</div>
                                                    <div className="text-xs text-gray-500">Oleh: {item.nama_user}</div>
                                                    <div className="text-xs text-gray-500">Ke: {item.lokasi_nama}</div>
                                                </td>
                                                <td className="px-6 py-4 text-sm whitespace-nowrap">
                                                ...
                                                <td className="px-6 py-4 text-sm whitespace-nowrap">
                                                    <div className="font-semibold text-gray-900">{item.kondisi}</div>
                                                    <div className="text-xs text-gray-500">{item.status_saat_kembali}</div>
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                        ...
                        {barangKembaliData.data.length > 0 && <div className="border-t p-4">{<Pagination links={barangKembaliData.links} />}</div>}
                    </div>
                </div>
            </div>
        </AppLayout>
